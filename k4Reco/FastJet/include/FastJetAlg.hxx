...
private:
  ...
  Gaudi::Property<std::vector<std::string>> m_jetAlgoNameAndParams{
      this, "algorithm", defaultJetAlgoNameAndParams,
      "Selects the algorithm and its parameters. E.g. 'kt_algorithm 0.7' or 'ee_kt_algorithm'. For a full list of "
      "supported algorithms, see the logfile after execution."};
  Gaudi::Property<std::vector<std::string>> m_clusterModeNameAndParam{
      this, "clusteringMode", defaultClusterMode,
      "One of 'Inclusive <minPt>', 'InclusiveIterativeNJets <nrJets> <minE>', 'ExclusiveNJets <nrJets>', "
      "'ExclusiveYCut <yCut>'. Note: not all modes are available for all algorithms."};
...
bool FastJetAlg::isJetAlgo(std::string algo, int nrParams, int supportedModes) const {
  ...
  if ((int)m_jetAlgoNameAndParams.size() - 1 != nrParams) {
    ...
    throw GaudiException("You have insufficient number of parameters for this algorithm! See log for more details.",
  ...
  if ((supportedModes & m_clusterMode) != m_clusterMode) {
    error() << std::endl
            << "This algorithm is not capable of running in this clustering mode (" << m_clusterMode << "). Sorry!"
            << std::endl;
