...
FastJetAlg::FastJetAlg(const std::string& name, ISvcLocator* svcLoc)
    : MultiTransformer(name, svcLoc, {KeyValues("recParticleIn", {"MCParticle"})},
                       {KeyValues("jetOut", {"JetOut"}), KeyValues("recParticleOut", {"Constituents"})}),
      m_jetAlgoName(""), m_jetAlgo(NULL), m_jetAlgoType(), m_clusterModeName(""), m_clusterMode(NONE),
      m_jetRecoScheme(), m_strategyName(""), m_strategy(), m_requestedNumberOfJets(0), m_yCut(0.0), m_minPt(0.0),
      m_minE(0.0) {}
...
StatusCode FastJetAlg::initialize() {
  ...
  if (m_jetRecoSchemeName.value().compare("E_scheme") == 0)
    ...
    throw GaudiException("Unknown FastJet recombination scheme! See log for more details.", name(),
                         StatusCode::FAILURE);
  }
  ...
  if (m_clusterModeName.compare("Inclusive") == 0) {
    if (m_clusterModeNameAndParam.size() != 2) {
      ...
      throw GaudiException("Wrong Parameter(s) for Clustering Mode. Expected:\n <parameter name=\"clusteringMode\" "
                           "type=\"StringVec\"> Inclusive <minPt> </parameter>",
    ...
    if (m_clusterModeNameAndParam.size() != 3) {
      throw GaudiException("Wrong Parameter(s) for Clustering Mode. Expected:\n <parameter name=\"clusteringMode\" "
                           "type=\"StringVec\"> InclusiveIterativeNJets <NJets> <minE> </parameter>",
                           name(), StatusCode::FAILURE);
    }
    ...
    if (m_clusterModeNameAndParam.size() != 2) {
      throw GaudiException("Wrong Parameter(s) for Clustering Mode. Expected:\n <parameter name=\"clusteringMode\" "
                           "type=\"StringVec\"> ExclusiveNJets <NJets> </parameter>",
                           name(), StatusCode::FAILURE);
    }
    m_requestedNumberOfJets = atoi(m_clusterModeNameAndParam[1].c_str());
    ...
    if (m_clusterModeNameAndParam.size() != 2) {
      throw GaudiException("Wrong Parameter(s) for Clustering Mode. Expected:\n <parameter name=\"clusteringMode\" "
                           "type=\"StringVec\"> ExclusiveYCut <yCut> </parameter>",
                           name(), StatusCode::FAILURE);
    }
  ...
  m_jetAlgo = NULL;
  info() << "Algorithms: "; // the isJetAlgo function will write to streamlog_out(MESSAGE), so that we get a list of
                            // available algorithms in the log
  // example: kt_algorithm, needs 1 parameter, supports inclusive, inclusiveIterative, exlusiveNJets and exlusiveYCut
  // clustering
  if (isJetAlgo("kt_algorithm", 1, FJ_inclusive | FJ_exclusive_nJets | FJ_exclusive_yCut | OWN_inclusiveIteration)) {
    ...
    m_jetAlgo =
        new fastjet::JetDefinition(m_jetAlgoType, atof(m_jetAlgoNameAndParams[1].c_str()), m_jetRecoScheme, m_strategy);
  }
  ...
  if (isJetAlgo("genkt_algorithm", 2, FJ_inclusive | OWN_inclusiveIteration | FJ_exclusive_nJets | FJ_exclusive_yCut)) {
    ...
    m_jetAlgo = new fastjet::JetDefinition(m_jetAlgoType, atof(m_jetAlgoNameAndParams[1].c_str()),
                                           atof(m_jetAlgoNameAndParams[2].c_str()), m_jetRecoScheme, m_strategy);
  }
  ...
  if (isJetAlgo("ee_kt_algorithm", 0, FJ_exclusive_nJets | FJ_exclusive_yCut)) {
    ...
    m_jetAlgo = new fastjet::JetDefinition(m_jetAlgoType, m_jetRecoScheme, m_strategy);
  }
  ...
  if (isJetAlgo("SISConePlugin", 2, FJ_inclusive | OWN_inclusiveIteration)) {
    ...
    pl = new fastjet::SISConePlugin(atof(m_jetAlgoNameAndParams[1].c_str()), atof(m_jetAlgoNameAndParams[2].c_str()));
  ...
  if (isJetAlgo("SISConeSphericalPlugin", 2, FJ_inclusive | OWN_inclusiveIteration)) {
    ...
    pl = new fastjet::SISConeSphericalPlugin(atof(m_jetAlgoNameAndParams[1].c_str()),
  ...
  try {
    if (m_clusterMode == FJ_inclusive) {
      ...
      if (inputCollection.size() < (int)m_requestedNumberOfJets) {
        ...
        double RDiff = R / 2; // the step size we modify the R parameter at each iteration. Its size for the n-th step
                              // is R/(2n), i.e. starts with R/2
        ...
        for (iIter = 0; iIter < ITERATIVE_INCLUSIVE_MAX_ITERATIONS; iIter++) {
          // do the clustering for this value of R. For this we need to re-initialize the JetDefinition, as it takes the
          // R parameter
