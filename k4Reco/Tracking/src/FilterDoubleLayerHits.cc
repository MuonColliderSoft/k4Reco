...
FilterDoubleLayerHits::FilterDoubleLayerHits(const std::string& name, ISvcLocator* svcLoc)
    : Transformer(name, svcLoc, KeyValues("InputCollection", {"VXDTrackerHitPlanes"}),
...
dd4hep::rec::Vector2D FilterDoubleLayerHits::globalToLocal(long int cellID, const dd4hep::rec::Vector3D& posGlobal,
                                                           dd4hep::rec::ISurface** surfptr = nullptr) const {
  ...
  if (surfptr && *surfptr) {
    ...
    if (surfIt == m_map->end()) {
      throw GaudiException(" FilterDoubleLayerHits::processEvent(): no surface found for cellID: " + cellID, "Fail",
                           StatusCode::FAILURE);
    }
    surf = surfIt->second;
...
StatusCode FilterDoubleLayerHits::initialize() {
  ...
  if (!m_map) {
    throw GaudiException(" Could not find surface map for detector: " + m_subDetName + " in SurfaceManager", "Fail",
                         StatusCode::FAILURE);
  }
  ...
  for (size_t iCut = 0, nCuts = m_dlCuts.size(); iCut < nCuts; ++iCut) {
    ...
    debug() << iCut << ". layers: " << cut.layer0 << " >> " << cut.layer1 << ";  dPhi: " << cut.dPhi_max
            << " rad;  dTheta: " << cut.dTheta_max << " rad" << endmsg;
  }
...
FilterDoubleLayerHits::operator()(const edm4hep::TrackerHitPlaneCollection& inputTrackerHitCollection) const {
  ...
  for (size_t iHit = 0; iHit < nHit; iHit++) {
    ...
    unsigned int ladderID = bitFieldCoder.get(h.getCellID(), "module");
    unsigned int moduleID = bitFieldCoder.get(h.getCellID(), "sensor");
    debug() << " Checking 1st hit " << iHit << " / " << nHit << " at layer: " << layerID << "  ladder: " << ladderID
            << "  module: " << moduleID << endmsg;
    ...
    if (hitsGrouped.find(sensPos2) == hitsGrouped.end())
      continue;
    for (size_t iHit2 : hitsGrouped.at(sensPos2)) {
      ...
      if (m_dtMax >= 0.0 && std::fabs(dt) > m_dtMax)
        continue;
...
StatusCode FilterDoubleLayerHits::finalize() { return StatusCode::SUCCESS; }